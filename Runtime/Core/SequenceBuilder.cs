...
{
    /// <summary>
    /// Fluent and safe sequence builder.
    /// Prevents uninitialized array slots that could execute Event 0.
    /// </summary>
    public class SequenceBuilder
    {
        ...
        public SequenceBuilder ThenParallel(EventData[] eventData)
        {
        ...
        {
            // Create exact-sized arrays; no uninitialized slots
            var actionsArray = _actions.ToArray();
            var singleArray = _singleSteps.ToArray();
            var parallelArray = _parallelSteps.ToArray();
            var waitArray = _waitSteps.ToArray();
            var delaysArray = _delays.ToArray();
